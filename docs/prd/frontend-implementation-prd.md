...
24. As a Forest Officer, I want the system to display Degraded Data Status when OpenWeather or other integrations fail, and block unlabeled live assessments when weather data is unavailable, so that I'm never presented with stale data labeled as live.
